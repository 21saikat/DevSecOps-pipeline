...
**curl \-sfL https://github.com/aquasecurity/trivy/releases/download/v0.41.0/trivy\_0.41.0\_Linux-64bit.deb \-o trivy.deb**
...
**7 . Go too sonarqube :**  Administration \> configuration \> webhook ( for create webhook )
...
pipeline {  
    agent any  
    environment {  
        SONAR\_HOME \= tool "Sonar" // Ensure "Sonar" is configured in Jenkins Global Tool Configuration  
    }  
    stages {  
...
pipeline {  
    agent any  
    environment {  
        SONAR\_HOME \= tool "Sonar" // SonarQube installation name in Jenkins Global Tool Configuration  
    }  
    stages {  
        stage("Clone Code from GitHub") {  
            steps {  
            ...
            steps {  
                withSonarQubeEnv("Sonar") { // Use the SonarQube server name configured in Jenkins  
                    sh """  
                        ...
                        \-Dsonar.login=squ\_56f9300841372df6d6ecc05a07ac95503ad5d6f2   
                    """  
                }  
            }  
        }  
            ...
            steps {  
                withSonarQubeEnv("Sonar") { // Use the SonarQube server name configured in Jenkins  
                    sh """  
                        ...
                        \-Dsonar.login=squ\_001c9d0dfb252f9a11cf1fe7c106688fdcaedfb8  
                    """  
                }  
            }  
        }  
